...
public class DesktopOverlay : MonoBehaviour
{
    ...
    // SetWindowPos 플래그
    private static readonly IntPtr HWND_BOTTOM    = new IntPtr(1);  // ✅ 수정: 항상 아래
    private static readonly IntPtr HWND_NOTOPMOST = new IntPtr(-2); // ✅ 수정: Topmost 해제
    private const uint SWP_NOMOVE      = 0x0002;
    private const uint SWP_NOSIZE      = 0x0001;
    private const uint SWP_FRAMECHANGED = 0x0020; // 스타일 변경 후 프레임 갱신
    private const uint SWP_NOACTIVATE  = 0x0010;

    // WinEvent 훅 — Z-order 변경 감지용 ✅ 추가
    ...
    // GC가 훅 델리게이트를 수집하지 못하도록 레퍼런스 유지 ✅ 추가
    ...
    private void Start()
    {
        ...
        // ✅ 수정: HWND_TOPMOST 대신 HWND_BOTTOM으로 바탕화면 위 고정
        ...
        // ✅ 추가: Z-order 변경 시에만 HWND_BOTTOM 재적용 (폴링 대신 이벤트 훅)
